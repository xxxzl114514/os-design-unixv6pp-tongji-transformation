...
namespace ELF
{
    ...
    int Parser::load(Inode *inode)
    {
        ...
        if (
            elfHeader32.magic[0] != 0x7F || elfHeader32.magic[1] != 'E' || elfHeader32.magic[2] != 'L' || elfHeader32.magic[3] != 'F')
        {
...
#define NUM_MIN(a, b) (a < b ? a : b)
        ...
        for (int idx = 0; idx < elfHeader32.phnum; idx++)
        {
            ...
            if (
                (ent.flags & PROGRAM_HEADER32_FLAGS_PF_R_BIT) && !(ent.flags & PROGRAM_HEADER32_FLAGS_PF_W_BIT))
            {
                // is read-only thing.
