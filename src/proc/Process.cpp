...
void Process::Exit()
{
	...
	{
		File *pFile = nullptr;
		if ((pFile = u.u_ofiles.GetF(i)) != nullptr)
		{
			fileTable.CloseF(pFile);
			u.u_ofiles.SetF(i, nullptr);
		}
	}
	...
	/* 释放该进程对共享正文段的引用 */
	if (u.u_procp->p_textp != nullptr)
	{
		u.u_procp->p_textp->XFree();
		u.u_procp->p_textp = nullptr;
	}
...
void Process::SStack()
{
	...
	p_size = oldSize + PageManager::PAGE_SIZE; // newSize
...
void Process::SBreak()
{
	...
	if (change < 0)
	{
		for (int i = last_page; i < first_page; i++)
		{
			...
			userPgMgr.FreeMemory(PageManager::PAGE_SIZE, entrys[i].m_PageBaseAddress << 12);
		}
	}
	else
	{
		...
		for (int i = first_page; i <= last_page; i++) // 没考虑物理内存不足的情况
		{
			...
			entrys[i].m_PageBaseAddress = (userPgMgr.AllocMemory(PageManager::PAGE_SIZE) >> 12);
		}
	}
