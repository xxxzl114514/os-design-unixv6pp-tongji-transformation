...
unsigned long BitMapAllocator::Alloc(BitMap &bitmap, unsigned long size)
{
	...
	// 遍历位图寻找连续的空闲页框
	int start = -1, count = 0;
	for (int i = 0; i < (bitmap.rows * 64 + bitmap.pages_left); i++)
	{
		if (is_free(bitmap, i))
		{
			if (count == 0)
...
int SwapperAllocator::is_free(SwapperBitMap &bitmap, int index)
{
	return !(bitmap.map[index / 64] & (1ULL << (index % 64)));
}

void SwapperAllocator::set_bit(SwapperBitMap &bitmap, int index, int value)
{
	if (value)
		bitmap.map[index / 64] |= (1ULL << (index % 64)); // 置 1
	else
...
SwapperAllocator SwapperAllocator::m_Instance_bitmap;

SwapperAllocator &SwapperAllocator::GetInstance()
{
	return SwapperAllocator::m_Instance_bitmap;
}

unsigned long SwapperAllocator::Alloc(SwapperBitMap &bitmap, unsigned long size)
{
	unsigned long retIdx = 0;
	...
	retIdx = start + bitmap.m_AddressIdx;
	return retIdx;
}

unsigned long SwapperAllocator::Free(SwapperBitMap &bitmap, unsigned long size, unsigned long addrIdx)
{
	int start_block = addrIdx - bitmap.m_AddressIdx;
	int blocks_to_free = size;

	for (int i = 0; i < blocks_to_free; i++)
	{
		set_bit(bitmap, start_block + i, 0);
	}
	return 0;
}