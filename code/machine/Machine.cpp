...
void Machine::InitPageDirectory()
{
	...
	PageDirectory *pPageDirectory = (PageDirectory *)(PAGE_DIRECTORY_BASE_ADDRESS +
													  KERNEL_SPACE_START_ADDRESS + 2 * KERNEL_SPACE_PAGE_SIZE);
...
void Machine::InitVESAMemoryMap(uintptr_t videoMemAddr, uintptr_t virtualMemAddr, size_t videoMemSize)
{
	...
	PageDirectory *pageDir = (PageDirectory *)(KERNEL_PAGE_TABLE_ZERO_DIRECTORY + KERNEL_SPACE_START_ADDRESS);
	...
	__asm(
		"mov %%eax, %%cr3"
		:
		: "a"(Machine::KERNEL_PAGE_TABLE_ZERO_DIRECTORY));
}

void Machine::InitVesaPageDirectory(
	uintptr_t videoMemAddr, uintptr_t virtualMemAddr,
	size_t videoMemSize, PageDirectory *pageDirectory)
{
	/********
	 * Note by chr
	 *
	 * 我们发现，上面的Vesa初始化会在页目录的第800个页目录初始化一个给vesa用的页表，
	 * 我们只需要把传入参数 pageDirectory 对应的位置正常写入即可
	 */
	uintptr_t videoMemBegin = videoMemAddr;
	videoMemBegin /= PageTable::SIZE_PER_PAGETABLE_MAP;
	videoMemBegin *= PageTable::SIZE_PER_PAGETABLE_MAP;
	...
	{
		uintptr_t vAddr = addr + virtualMemBegin - videoMemBegin;
		auto &entry = pageDirectory->m_Entrys[vAddr / PageTable::SIZE_PER_PAGETABLE_MAP];
		entry.m_UserSupervisor = 0;
		entry.m_Present = 1;
		entry.m_ReadWriter = 1;
		entry.m_PageSize = 1;
...
void Machine::InitUserPageTable()
{
	...
	PageTable *pUserPageTable = (PageTable *)(USER_PAGE_TABLE_BASE_ADDRESS + KERNEL_SPACE_START_ADDRESS);
	// PageTable *pUserPageTable =(PageTable *)(USER_PAGE_TABLE_BASE_ADDRESS);
	...
	PageTable *pUserPageTable2_Base_Address = (PageTable *)((unsigned long)pPageDirectory - KERNEL_SPACE_START_ADDRESS + 0x1000);
	pUserPageTable =
		(PageTable *)((unsigned long)pUserPageTable2_Base_Address + KERNEL_SPACE_START_ADDRESS);
	idx = (unsigned long)pUserPageTable2_Base_Address >> 12;
	...
	for (unsigned int i = 0; i < PageTable::ENTRY_CNT_PER_PAGETABLE; i++)
	{
		pUserPageTable->m_Entrys[i].m_UserSupervisor = 0;
		pUserPageTable->m_Entrys[i].m_Present = 0;
		pUserPageTable->m_Entrys[i].m_ReadWriter = 0;
		pUserPageTable->m_Entrys[i].m_WriteThrough = 0;
		pUserPageTable->m_Entrys[i].m_CacheDisabled = 0;
		pUserPageTable->m_Entrys[i].m_Accessed = 0;
		pUserPageTable->m_Entrys[i].m_Dirty = 0;
		pUserPageTable->m_Entrys[i].m_PageTableAttribueIndex = 0;
		pUserPageTable->m_Entrys[i].m_GlobalPage = 0;
		pUserPageTable->m_Entrys[i].m_ForSystemUser = 0;
		pUserPageTable->m_Entrys[i].m_PageBaseAddress = 0x00000 + 0 + 0;
	}
...
void Machine::EnablePageProtection()
{
	...
	unsigned int pd = (unsigned int)(KERNEL_PAGE_TABLE_ZERO_DIRECTORY);
...
void Machine::SetPageDirectory(PageDirectory *pPageDirectory)
{
	this->m_PageDirectory = pPageDirectory;
}
void Machine::SetUserPageTableArray(PageTable *pUserPageTableArray)
{
	this->m_UserPageTable = pUserPageTableArray;
}

void Machine::InitZeroProcUserTable()
{
	PageDirectory *pPageDirectory = this->m_PageDirectory;
	...
	for (unsigned int i = 0; i < PageTable::ENTRY_CNT_PER_PAGETABLE; i++)
	{
		...
		pUserPageTable[0].m_Entrys[i].m_Present = 1;
		pUserPageTable[0].m_Entrys[i].m_ReadWriter = 1;
		pUserPageTable[0].m_Entrys[i].m_PageBaseAddress = 0x00000 + i + 0 * 1024;
	}
	...
	for (unsigned int i = 0; i < PageTable::ENTRY_CNT_PER_PAGETABLE; i++)
	{
		...
		pUserPageTable->m_Entrys[i].m_Present = 0;
		pUserPageTable->m_Entrys[i].m_ReadWriter = 0;
		pUserPageTable->m_Entrys[i].m_PageBaseAddress = 0x00000;
	}
