...
#define SwtchUStruct(p)                                                                                                                  \
	Machine::Instance().GetKernelPageTable().m_Entrys[Kernel::USER_PAGE_INDEX].m_PageBaseAddress = (p)->p_addr / PageManager::PAGE_SIZE; \
...
#define RetU()                                         \
	__asm__ __volatile__("	movl $0xc03ff000, %%eax;	\
							movl (%%eax), %%esp;		\
							movl 0x4(%%eax), %%ebp;" \
						 : : : "eax");
...
protected:
	void cleanPageDirectory(PageDirectory *pPageDirectory);
};
